...
export const getConnectionsAPI = async () => {
  const response = await axios.get(`http://localhost:5000/api/connections/myconnections`,{
    headers: {
...
export const getUserByIdAPI= async (userId) => {
    const response = await axios.get(`http://localhost:5000/api/connections/${userId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const createConnectionAPI= async (userId) => {
  const response = await axios.post(`http://localhost:5000/api/connections/${userId}/connect`,{},{
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const removeConnectionAPI= async (userId) => {
    const response = await axios.post(`http://localhost:5000/api/connections/${userId}/remove`,{},{
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
